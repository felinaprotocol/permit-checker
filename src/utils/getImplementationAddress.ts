...
export async function getImplementationAddress(provider: ethers.providers.JsonRpcProvider, proxyAddress: string): Promise<string | null> {
    ...
    if (contractByteCode.startsWith('0x363d3d373d3d3d363d73')) {
        ...
        if (endSequenceIndex > -1) {
            ...
            return ethers.utils.getAddress(implementationAddress);
        }
    }
    ...
    for (const slot of storageSlots) {
        try {
            const implementationAddress = await provider.getStorageAt(proxyAddress, slot);
            if (implementationAddress !== '0x0000000000000000000000000000000000000000') {
                return ethers.utils.getAddress(ethers.utils.hexStripZeros(implementationAddress));
            }
        } catch (error) {
            continue;
        }
